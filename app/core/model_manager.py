...
class ModelManager:
    ...
    def resolve_model_path(self, model_key: str) -> Path:
        ...
        custom_user = self.user_models_dir / f"ggml-{model_key}.bin"
        if custom_user.exists():
            return custom_user
        custom_bundled = self.bundled_dir / f"ggml-{model_key}.bin"
        if custom_bundled.exists():
            return custom_bundled
        raise FileNotFoundError(f"Model not found: {model_key}")
    ...
    def installed_model_labels(self) -> list[str]:
        labels = []
        for key, path, source in self.available_models():
            label = MODEL_DEFS.get(key).label if key in MODEL_DEFS else key
